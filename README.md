...
本项目用于通过阿里云 RAM 只读策略和 `aliyun` CLI 梳理云资产，默认输出原始资源中心清单、可交付 Excel 报告，以及 PlantUML 网络拓扑。
...
默认会按 VPC 输出 PlantUML 网络拓扑；如需显式写法也可以：
...
- `README.md`：拓扑文件索引。
- `<region>__<vpc-id>.md`：单个 VPC 的 PlantUML 网络拓扑（嵌入 `@startuml/@enduml` 代码块）。
- `unassigned.md`：无法确认 VPC 归属的资源。

PlantUML 代码块可直接在 [PlantUML Online Server](https://www.plantuml.com/plantuml/uml) 渲染，也可使用 IDE 插件（VS Code PlantUML 插件、IntelliJ PlantUML integration 等）或命令行 `plantuml` 工具渲染为 PNG/SVG。
...
| `--topology`           | 显式开启按 VPC 切分的 PlantUML 网络拓扑 Markdown，默认已开启                             |
| `--no-topology`        | 关闭 PlantUML 网络拓扑输出                                                               |
...
无论从项目根目录还是 `scripts/` 目录执行，默认输出目录都会固定到项目根目录的 `outputs/`。相对路径参数，例如 `--output-dir outputs/smoke`、`--checks-config config/checks.example.json`，也会按项目根目录解析。默认会输出 PlantUML 拓扑文件到 `--output-dir/<订阅目录>/topology/`；如不需要，可加 `--no-topology`。
