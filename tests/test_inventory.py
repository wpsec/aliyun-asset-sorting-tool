...
class TopologyRenderingTest(unittest.TestCase):
    def test_build_topology_plantuml_documents_renders_basic_vpc_relationships(self):
        raw_rows = [
            raw_row(
        ...
        docs = inventory.build_topology_plantuml_documents(raw_rows, details)
        ...
        self.assertIn("VSW_vsw_1", content)
        self.assertIn("ECS_i_1", content)
        self.assertIn("EIP_eip_1", content)
        self.assertIn("EIP_eip_1 --> ECS_i_1", content)
        self.assertIn("绑定", content)
        self.assertIn("安全组: sg-1,sg-2", content)

    def test_build_topology_plantuml_documents_renders_alb_chain_without_guessing(self):
        raw_rows = [
            raw_row(
                resource_type="ACS::VPC::VPC",
                resource_type_name="专有网络",
        ...
        self.assertIn("LIS_lsn_1", content)
        self.assertIn("SG_sg_1", content)
        self.assertIn("ECS_i_backend", content)
        self.assertIn("LIS_lsn_1 --> SG_sg_1", content)
        self.assertIn("SG_sg_1 ..> ECS_i_backend", content)
        self.assertNotIn("LIS_lsn_2 --> SG_sg_1", content)

    def test_build_topology_plantuml_documents_puts_unassigned_database_into_unassigned(self):
        raw_rows = [
            raw_row(
                resource_type="ACS::VPC::VPC",
                resource_type_name="专有网络",
    ...
    def test_build_topology_plantuml_documents_marks_partial_relationships(self):
        raw_rows = [
            raw_row(
                resource_type="ACS::VPC::VPC",
                resource_type_name="专有网络",
